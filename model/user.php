...
// hàm, lệnh SQL để tương tác vs CSDL
    ...
    //đăng nhập
    function login($email,$password){ // tham số truyền 
        global $conn;
        $sql = "SELECT * FROM taikhoan WHERE email='".$email."' AND matkhau='".$password."'"; // lấy ra tài khoản 
        $conn = pdo_get_connection();
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        return $stmt->fetch(); // lấy 1 cái 
    }  
    //đăng ký
    function signup($hoten,$email,$matkhau){
        global $conn;
        $sql = "INSERT INTO taikhoan(`hoten`,`email`,`matkhau`) VALUES (:hoten,  :email, :matkhau)";
        $conn = pdo_get_connection(); // kết nối tới  cơ sở dữ liệu 
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(":hoten",$hoten); // buộc giữ liệu 
        $stmt->bindParam(":email",$email);
        $stmt->bindParam(":matkhau",$matkhau);
        return $stmt->execute(); // không láy dữ liệu nên ko lấy hàm fetch //kết quả đang ki có thành công hay không 
    }
    //check sdt tồn tại
    ...
    function add_user($hoten, $anh, $email, $diachi, $sdt,$matkhau ,$quyen){
        pdo_execute("INSERT INTO taikhoan(`hoten`,`anh`,`email`,`diachi`,`sdt`,`matkhau`,`quyen`)VALUES(?,?,?,?,?,?,?)",$hoten, $anh, $email, $diachi, $sdt,$matkhau ,$quyen);
    }
    //SỬA TÀI KHOẢN
    function edit_user($makh, $hoten, $anh, $email, $diachi, $sdt, $matkhau, $quyen){
        pdo_execute("UPDATE taikhoan SET hoten=?, anh=?, email=?, diachi=?, sdt=?, matkhau=?, quyen=? WHERE makh=?",$hoten, $anh, $email, $diachi, $sdt, $matkhau, $quyen, $makh);
    }
    //xoá tài khoản
